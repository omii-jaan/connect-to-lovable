...
import { Github, Lock, Mail, Terminal, Zap, UserCheck } from "lucide-react";
...
const Login = () => {
  ...
  const { user, signInWithGithub, signInWithGoogle, switchTestUser, activeTestKey } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const redirectPath = (location.state as { from?: { pathname: string } })?.from?.pathname || new URLSearchParams(location.search).get("redirect") || "/dashboard";
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden px-6 py-16 bg-background">
      ...
      <main id="main" className="relative z-10 w-full max-w-md">
        ...
        {/* Terminal auth card */}
        <div className="relative rounded-3xl border border-border-subtle bg-card shadow-elev-lg overflow-hidden">
          {/* Terminal title bar */}
          <div className="flex items-center justify-between px-5 py-3 border-b border-border-subtle bg-card">
          ...
          {/* Terminal log area */}
          <div ref={logContainerRef} className="px-5 pt-4 pb-2 bg-muted max-h-28 overflow-y-auto border-b border-border-subtle">
            {logLines.map((line, i) => (
              <p
                key={i}
          ...
          <div className="p-5 sm:p-6">
            ...
            {/* OAuth & Quick Demo Auth as terminal commands */}
            <div className="space-y-2 mb-5">
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  disabled={loading}
                  className="group relative flex items-center gap-2.5 px-4 py-3 rounded-xl bg-background/40 border border-border-subtle text-left font-mono text-xs text-muted-foreground hover:text-foreground hover:border-primary/30 hover:bg-primary/[0.04] transition-all disabled:opacity-50"
                >
              ...
              <div className="p-3 rounded-xl bg-muted/40 border border-border/40">
                <p className="text-[10px] font-mono text-muted-foreground mb-2 flex items-center justify-between">
                  <span>1-Click Test Builder Dock:</span>
                  <span className="text-primary font-semibold">Instant Auth</span>
                </p>
                <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                  <button
                    type="button"
                    onClick={() => switchTestUser("user_a")}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-card border border-border hover:border-primary/50 text-foreground transition-all"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-card border border-border hover:border-primary/50 text-foreground transition-all"
                  >
                    <UserCheck className="w-3.5 h-3.5 text-secondary-bright" />
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none pt-0.5">
                ...
                />
                <span className="w-3.5 h-3.5 rounded border border-border bg-foreground/5 peer-checked:bg-primary peer-checked:border-primary flex items-center justify-center transition-all shrink-0">
                  {remember && (
